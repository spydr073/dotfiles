...
##   - never: Never send the Referer. This is not recommended, as
...
##     (letter, word or number mode) or the element's text (only in number mode).
...
##   - tab-bg-silent: Open a new background tab in the existing window without activating
##     the window.
...
## Type: String
## Valid values:
##   - software-opengl: Tell LibGL to use a software implementation
##     of GL (`LIBGL_ALWAYS_SOFTWARE` / `QT_XCB_FORCE_SOFTWARE_OPENGL`)
...
## Type: List of String
c.url.yank_ignored_parameters = [ 'ref'
                                , 'utm_source'
                                , 'utm_medium'
                                , 'utm_campaign'
                                , 'utm_term'
...
config.bind('<Alt-t>'  , 'set-cmd-text -s :tab-focus')
